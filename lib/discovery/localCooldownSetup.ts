...
export async function expireLocalProductDiscoveryCooldown(projectId: string): Promise<void> {
  ...
  execFileSync(
    ...
    [
      ...
      `update public.project_ai_request_guards
       set last_started_at = now() - interval '61 seconds'
       where project_id = '${parsed.data}'::uuid
         and operation = 'product_discovery';`,
    ],
