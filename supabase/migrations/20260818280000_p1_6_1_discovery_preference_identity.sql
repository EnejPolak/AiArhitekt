-- P1.6.1: discovery source identity includes shopping-affecting preferences.
-- Does not rewrite P1.1–P1.7c history.
-- Does not generate a renovation render or change provider calls.

alter table public.project_product_discoveries
...
alter table public.project_product_discoveries
  add column if not exists source_preferences_hash text not null default
...
set
  ...
  source_preferences_hash = 'e4c7458abb77f59d62720fa17e2e3a041739c6c6a884f9938447753345f4122f'
where source_preferences_hash is null
   or btrim(source_preferences_hash) = '';

alter table public.project_product_discoveries
  drop constraint if exists project_product_discoveries_source_preferences_is_object;

alter table public.project_product_discoveries
  add constraint project_product_discoveries_source_preferences_is_object
  check (jsonb_typeof(source_preferences) = 'object');

alter table public.project_product_discoveries
  drop constraint if exists project_product_discoveries_source_preferences_hash_len;

alter table public.project_product_discoveries
  add constraint project_product_discoveries_source_preferences_hash_len
  check (char_length(source_preferences_hash) = 64);

comment on column public.project_product_discoveries.source_preferences is
  'Normalized shopping-affecting preference snapshot used for this discovery. Audit why the run is current. Not style-only UI state.';
comment on column public.project_product_discoveries.source_preferences_hash is
  'SHA-256 hex of canonical JSON for source_preferences. Discovery is reusable only when this matches the current normalized preferences.';
comment on table public.project_product_discoveries is
  'One current product discovery run per project. Current only when source analysis identity, normalized location, and source_preferences_hash all match. MAX_PRODUCT_DISCOVERY_ITEMS = 10.';
...
comment on function public.replace_project_product_discovery_result(uuid, uuid, jsonb, jsonb) is
  'Trusted SERP persist for one project. Writes analysis identity, location, and shopping preference snapshot/hash. SECURITY DEFINER, service_role only.';
